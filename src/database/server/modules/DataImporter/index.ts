    ...
    await serverDB.transaction(async (trx) => {
      if (data.sessionGroups && data.sessionGroups.length > 0) {
        ...
        const mapArray = await trx
          ...
          .onConflictDoUpdate({ set: { updatedAt: new Date() }, target: sessionGroups.clientId })
      ...
      if (data.sessions && data.sessions.length > 0) {
        ...
        const mapArray = await trx
          ...
          .onConflictDoUpdate({ set: { updatedAt: new Date() }, target: sessions.clientId })
      ...
      if (data.topics && data.topics.length > 0) {
        ...
        const mapArray = await trx
          ...
          .onConflictDoUpdate({ set: { updatedAt: new Date() }, target: topics.clientId })
